...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 1D69657A1DA
	for <lists+linux-pci@lfdr.de>; Tue, 19 Jul 2022 16:39:04 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S239418AbiGSOjB (ORCPT <rfc822;lists+linux-pci@lfdr.de>);
        Tue, 19 Jul 2022 10:39:01 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:59164 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S239107AbiGSOiZ (ORCPT
        <rfc822;linux-pci@vger.kernel.org>); Tue, 19 Jul 2022 10:38:25 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 7237245F76;
        Tue, 19 Jul 2022 07:33:42 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id C8E1A617D6;
        Tue, 19 Jul 2022 14:33:41 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id AB34AC341C6;
        Tue, 19 Jul 2022 14:33:40 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1658241221;
        bh=cKsrgqjIp2OmBa2yh0bh+nW8LwKcX0C6R7yttctaKJY=;
        h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
        b=
Received: by pali.im (Postfix)
        id B0D8BF3C; Tue, 19 Jul 2022 16:33:37 +0200 (CEST)
Date:   Tue, 19 Jul 2022 16:33:37 +0200
...
Message-ID: <20220719143337.7wzvqkesyxnbp5pn@pali>
...
In-Reply-To: <BN9PR18MB4251ABE705A42DD17DA2DE23DB8F9@BN9PR18MB4251.namprd18.prod.outlook.com>
...
I was not able to find any such register too... but it is possible that
I just do not have "complete" documentation. For example I was able to
find this register for A7K/A8K/CN913X family, it is 0xF26008D0
(AMBA_ERROR_RESPONSE_DEFAULT_FF). And I think that similar register
should be also for A3720, but looks like it is not documented.

I have already played with register 0xD0074808 (PCIe Control 2).

Bit 5 is already being disabled during driver initialization and it is
described as required step in A3720 'PCIe Link Initialization'. But I do
not understand what it is 'PCIe strict ordering rules'. In PCIe we have
strong ordering or relaxed ordering; there is no strict ordering. It is
typo in documentation?

And bit 6 is used for enabling transparent outbound window
configuration and is enabled during driver initialization. I have
already tried to disable this bit and set outbound window configuration
manually (in action register). No visible change.

Do you have some contact for Marvell Support? Because we are unable to
reach any support/tech and there is just silence.

Also do you know where we can get information about registers which are
already used by Marvell driver but are not documented in documentation
to which I have access on Marvell portal?

On Tuesday 19 July 2022 13:45:32 Elad Nachman wrote:
> Hi Pali,
> 
> Unfortunately I did not find any registers matching the ones you were looking for.
> I can suggest is playing with the PCIe Control 2 register, address 0x74808.
> Bit 6 is the AXI Address Window Location Generation Enable bit, bit 5 controls the strict ordering (default - 1 - enable strict ordering) .
> Perhaps another option is changing the properties of the PCIe windows (Action register and default user field)
> 
> Regarding the Erratum, I have checked with my Manager and unfortunately the only legal way to access it is via the Marvell Support side. I regret that there is no better option at this time, hopefully this is understandable.
> 
> I agree with your technical recap for the workaround.
> 
> Elad.
> 
> -----Original Message-----
...
> A3720 ARM A53 core, because that register is in address space of secure ARM CM3 core, to which Linux kernel does not access.
> 
> I looked into my notes and bit 6 of register 0x40000408 controls AXI SLVERR propagation just from BCM slave to CM3 core. BCM is Base cryptographic module, which is used by the secure firmware running on
> CM3 core. So it looks like it is irrelevant to ARM A53 cores on which is running Linux kernel and which receive these crashes, and also does not apply for PCIe AXI slave which is connected A3720 South Fabric.
> I guess that register 0x40000408 controls only Fabric directly connected to ARM CM3 core.
> 
> So it is needed to know what is equivalent register for A53 and A3720 South Fabric. Or register specific for A3720 PCIe block, so for Unsupported Request, Completer Abort, Configuration request retry status and Completion Timeout does not throw AXI SLVERR to A53 core.
> 
> Whole description of errata is important for kernel developers / maintainers who will be reviewing code. We do not need whole document, just that one errata. It would really help if Marvell could get permission for sharing this errata for kernel developers. Because it is really a pain if every kernel developer has to go throw NDA/LULA process and wait for Sales team if they approve access or not.
> 
> In past documentation for Marvell SoCs was public and every developer was able to download it from marvell.com website without registration.
> But now all links to public documentation do not work anymore.
> Thankfully web.archive.org created backup of some of them and they are linked from Kernel documentation file:
> https://urldefense.proofpoint.com/v2/url?u=https-3A__www.kernel.org_doc_html_latest_arm_marvell.html&d=DwIDaQ&c=nKjWec2b6R0mOyPaz7xtfQ&r=eTeNTLEK5-TxXczjOcKPhANIFtlB9pP4lq9qhdlFrwQ&m=hjGMHtZh59hkfXFzDXs5D4vATYBFK9pcablt4NFvniM12B56b5twDkC4usCU5dAd&s=dRSFRXzcjo8StUkJ7AgW33Y2KzZu05tuGyOpbLVbb-c&e=
> (see how many documents were lost...)
> 
> Anyway, I have access to Marvell portal and some A3720 documents, but lot of parts in those documents to which I have access are incomplete or registers undocumented.
> 
> So to recap, workaround should be to disable ordering checks via DIS_ORD_CHK bit, adding DSB barrier when entering aardvark summary irq handler and ideally also enable relaxed ordering bit, so initiated transactions could use relaxed ordering when requested (as strong ordering does not work).
> 
...
> > To: Elad Nachman <enachman@marvell.com>
> > Cc: Ratheesh Kannoth <rkannoth@marvell.com>; Tanmay Jagdale 
> > <tanmay@marvell.com>; Shijith Thotton <sthotton@marvell.com>; Arun 
> > Easi <aeasi@marvell.com>; Krzysztof Wilczyński <kw@linux.com>; Lorenzo 
> > Pieralisi <lorenzo.pieralisi@arm.com>; Thomas Petazzoni 
...
> > Subject: Re: [EXT] Re: Issues with A3720 PCIe controller driver 
> > pci-aardvark.c
> > 
> > Hello Elad!
> > 
...
> > lEmbeddedProcessors_linux-2Dmarvell_blob_f12b3557095ff1d9b7be78bef7a32
> > 37e27b7778e_drivers_pci_host_pci-2Dadvk-2Darm64.c-23L397&d=DwIDaQ&c=nK
> > jWec2b6R0mOyPaz7xtfQ&r=eTeNTLEK5-TxXczjOcKPhANIFtlB9pP4lq9qhdlFrwQ&m=Q
> > oWBVtlmK8dcnqfqPuLlrYZW9dLkLAXjQsXnSTsyLXtAG5xwVrUj34pscwdHn0qv&s=dJOx
> > 9jStTL77pFhd5KB7ZjA-QJr1hryKyBaO8iykKQI&e=
> > 
...
> > > > > Thomas nor me were able to understood text of this erratum. And 
> > > > > we have already spent lot of time on this erratum. My guess that 
> > > > > is that in erratum itself are mistakes and there are missing some other important details.
> > > > > 
...
> > > > > 
> > > > > Also it would be useful if you / Marvell could share text of 
> > > > > this erratum with linux-pci people as currently it is available 
> > > > > only on Marvell Customer Portal which requires registration with signed NDA.
> > > > > 
> > > > > In past Thomas wrote patch "according to this erratum" and I 
> > > > > have rebased, rewritten and resent it to linux-pci mailing list for review:
> > > > > https://urldefense.proofpoint.com/v2/url?u=https-3A__lore.kernel
> > > > > .o
> > > > > rg
> > > > > _linux-2Dpci_20210624222621.4776-2D6-2Dpali-40kernel.org_&d=DwID
> > > > > aQ
> > > > > &c
> > > > > =nKjWec2b6R0mOyPaz7xtfQ&r=eTeNTLEK5-TxXczjOcKPhANIFtlB9pP4lq9qhd
> > > > > lF
> > > > > rw
> > > > > Q&m=opYsQsv_sfSvTtA5oJwc1paZrPAWMHVhTx_9J1VWBVDksBETCXVsC3rRDb5e
> > > > > jg
...
> > > > > DIS_ORD_CHK in that patch should be disabled. Seems that 
> > > > > enabling this bit effectively disables PCIe strong ordering 
> > > > > model. PCIe kernel drivers rely on PCIe strong ordering, so it 
> > > > > would implicate that that bit should not be enabled. Which is opposite of what is mentioned patch doing.
> > > > > 
